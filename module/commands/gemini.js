...
const API_KEY = "AIzaSyBLkdNAIICGPNgahtdIfQQlPeMKMw9mN-4";
...
module.exports.handleEvent = async ({ event, api, Users, Threads }) => {
  ...
  if (prompt || imageParts.length > 0) {
    try {
      ...
      conversationHistory[senderID].push(`Nguyễn Kim Ngân: ${text}`);

      await saveDataToFile();

      const delay = Math.floor(Math.random() * (15000 - 10000 + 1)) + 10000; // Delay từ 10 đến 15 giây
      setTimeout(() => {
...
module.exports.run = async ({ api, event, Threads, getText }) => {
  ...
  return api.sendMessage(`Chào anhhh :3`, threadID, messageID);
};